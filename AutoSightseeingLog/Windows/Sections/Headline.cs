...
internal static class Headline
{
    ...
    private static CachedText lastRunText;

    public static bool Draw(Configuration configuration, TourController controller, RunHistory history)
    {
        ...
        using (Fonts.PushTitle())
        {
            TextDraw.At(TextDraw.Truncate(info.Title + Loc.T(L.Plan.SentenceEnd), maxTextWidth), new Vector2(origin.X, y), Styling.TextStrong);
        }
    ...
    private static (string Title, string Detail) LastRun(RunHistory history)
    {
        ...
        if (records.Count == 0)
        {
            return (Loc.T(L.Vistas.NoRunsYet), Loc.T(L.Vistas.StatsAppearHere));
        }

        var record = records[0];
        var title = lastRunText.Get(record.VistasLogged, static key => Loc.Plural(L.Vistas.LastRun, (int)key));
        return (title, Formatting.Elapsed(record.Duration));
    }
}
