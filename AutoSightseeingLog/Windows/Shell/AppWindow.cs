using AutoSightseeingLog.Windows.Pages;
...
{
    public enum Page { Vistas, Settings, History, Plugins, About }
    ...
    private readonly VistaPage vistaPage = new();
    ...
    public AppWindow(Plugin plugin) : base("Auto Sightseeing Log###AutoSightseeingLogMain", BaseFlags)
    {
        this.plugin = plugin;
        Size = DefaultSize;
    ...
    private void DrawBody(Vector2 windowPos, Vector2 windowSize, float headerHeight, float windowRounding)
    {
        ...
        var dockHeight = page == Page.Vistas ? Layout.DockHeight * scale
        ...
        using (var rail = ImRaii.Child("##asl_rail", new Vector2(railWidth, bodyHeight), false, ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoScrollWithMouse))
        {
        ...
        using (var content = ImRaii.Child("##asl_page", new Vector2(windowSize.X - railWidth - rightInset, bodyHeight), false, ImGuiWindowFlags.AlwaysUseWindowPadding))
        {
            if (content) DrawPage();
        }
        ...
        if (page == Page.Vistas) ActionDock.Draw(plugin, dockSize, windowRounding);
        else if (MiniPlayer.Draw(plugin, dockSize, windowRounding)) Show(Page.Vistas);
    }
    ...
    private void DrawPage()
    {
        ...
        switch (page)
        {
            case Page.Vistas: vistaPage.Draw(plugin, this); break;
