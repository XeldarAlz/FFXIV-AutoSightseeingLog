...
{
    private const string Name = "Auto Sightseeing Log";
    ...
    private void DrawHero()
    {
        ...
        Paint.Gradient(dl, origin, max, Styling.Tint(Styling.Surface2, Styling.AccentStar, 0.20f), Styling.Tint(Styling.Surface0, Styling.AccentStar, 0.05f), rounding);
        DrawAurora(dl, origin, max);
        Paint.TopLight(dl, origin, max, rounding, 0.14f);
        Paint.Stroke(dl, origin, max, Styling.WithAlpha(Styling.AccentStar, 0.35f), rounding, 1.2f * scale);
    ...
    private static void DrawAurora(ImDrawListPtr dl, Vector2 min, Vector2 max)
    {
        ...
        SoftBlob(dl, min + new Vector2(width * (0.18f + 0.08f * Motion.Wave(11000)), height * (0.30f + 0.20f * Motion.Wave(13700))), height * 1.3f, Styling.AccentStar, 0.10f);
        SoftBlob(dl, min + new Vector2(width * (0.72f + 0.10f * Motion.Wave(15500)), height * (0.10f + 0.25f * Motion.Wave(9300))), height * 1.1f, Styling.AccentAether, 0.08f);
    ...
    private static void DrawHeroIcon(ImDrawListPtr dl, Vector2 center, float size)
    {
        var scale = ImGuiHelpers.GlobalScale;
        var half = size * 0.5f;
        ProgressRing.Glow(center, half * 1.05f, Styling.AccentStar, 0.5f + 0.45f * Styling.Pulse(Styling.PulseBreath));
        ProgressRing.Sweep(center, half * 1.28f, 2f * scale, Styling.AccentStarSoft, Styling.PulseOrbit, MathF.PI * 0.5f, 0.9f);
        OrbitParticles(dl, center, half * 1.28f, 3, 4600, 1, Styling.AccentStarSoft, 2.2f * scale);
        ...
        Paint.Stroke(dl, iconMin, iconMax, Styling.WithAlpha(Styling.AccentStarSoft, 0.55f), rounding, 1.5f * scale);
    ...
    private static void DrawHeroText(float x, float width, float top, float height)
    {
        ...
        using (Fonts.PushCaption())
        {
            StaticChip(FontAwesomeIcon.Tag, Loc.T(L.About.Version, version), Styling.AccentStar, new Vector2(x, y), chipHeight);
        }
    }
    ...
    private static void DrawShimmerTitle(string text, Vector2 position, float width)
    {
        ...
        TextDraw.At(text, position, Styling.AccentStarSoft);
        dl.PopClipRect();
        dl.PopClipRect();
    }
