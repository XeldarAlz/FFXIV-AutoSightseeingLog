...
internal sealed class ChangelogPage
{
    ...
    private void BeginVisit(long shownTick)
    {
        ...
        visitTick = shownTick;
        unseenAtVisit = ChangelogData.UnseenCount(Plugin.Instance.Configuration.LastSeenChangelogVersion);
        Plugin.Instance.Configuration.MarkChangelogSeen();
    }
    ...
    private void DrawEntry(int index, bool isLast)
    {
        ...
        if (isNew)
        {
            Paint.Glow(dl, cardMin, cardMax, rounding, Styling.AccentStar, 0.8f);
        }

        if (isLatest)
        {
            Paint.Glass(dl, cardMin, cardMax, rounding, Styling.AccentStar, 0.08f);
            Paint.Stroke(dl, cardMin, cardMax, Styling.WithAlpha(Styling.AccentStar, isNew ? 0.55f : 0.30f), rounding);
        }
        else
        {
        ...
        using (Fonts.PushHeadline())
        {
            TextDraw.At(versionLabels[index], new Vector2(x, cardMin.Y + padY), isLatest ? Styling.AccentStarSoft : Styling.TextStrong);
    ...
    private static void DrawRail(ImDrawListPtr dl, Vector2 origin, float dotY, float cardHeight, bool highlighted, bool isLast)
    {
        ...
        if (highlighted)
        {
            Paint.Dot(dl, center, radius, Styling.AccentStar, 0.28f);
            return;
        }
    ...
    private static float DrawPill(ImDrawListPtr dl, LocString label, float x, float midY, bool filled)
    {
        ...
        using (Fonts.PushCaption())
        {
            ...
            if (filled)
            {
                Paint.Pill(dl, min, max, Styling.AccentStar, Styling.WithAlpha(Styling.AccentStarSoft, 0.6f));
                TextDraw.At(text, new Vector2(min.X + PillPadX * scale, midY - size.Y * 0.5f), Styling.ForegroundOn(Styling.AccentStar));
            }
            else
            {
                Paint.Pill(dl, min, max, Styling.WithAlpha(Styling.AccentStar, 0.16f), Styling.WithAlpha(Styling.AccentStar, 0.4f));
                TextDraw.At(text, new Vector2(min.X + PillPadX * scale, midY - size.Y * 0.5f), Styling.AccentStarSoft);
            }
    ...
    private static void DrawBody(ImDrawListPtr dl, LocString[] highlights, float bulletX, float textLeft, float textWidth, float top)
    {
        ...
        for (var index = 0; index < highlights.Length; index++)
        {
            ...
            dl.AddCircleFilled(new Vector2(bulletX + BulletRadius * scale, y + lineHeight * 0.5f), BulletRadius * scale, Paint.Col(Styling.AccentStar));
