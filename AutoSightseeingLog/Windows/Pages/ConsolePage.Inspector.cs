...
internal sealed partial class ConsolePage
{
    ...
    private const string InspectorDetailId = "##asl_console_detail";
    ...
    private void DrawSelectionBar()
    {
        ...
        Paint.Surface(dl, origin, origin + new Vector2(width, height), Styling.CardRounding * scale,
            Styling.WithAlpha(Styling.AccentStar, 0.08f), Styling.WithAlpha(Styling.AccentStar, 0.35f));

        var (low, high) = SelectionIndices();
        var label = Loc.Plural(L.Console.Selected, high - low + 1);
        var labelSize = TextDraw.Measure(label);
        TextDraw.At(label, new Vector2(origin.X + CardPadX * scale, origin.Y + (height - labelSize.Y) * 0.5f), Styling.TextStrong);

        var clearLabel = Loc.T(L.Console.ClearSelection);
        var copyLabel = Loc.T(L.Console.CopySelection);
        ...
        if (PillButton.Draw("##asl_console_sel_clear", clearLabel, Styling.TextSecondary, PillButton.Emphasis.Ghost, FontAwesomeIcon.Times,
        ...
        if (PillButton.Draw("##asl_console_sel_copy", copyLabel, Styling.AccentStar, PillButton.Emphasis.Tinted, FontAwesomeIcon.Copy,
                height: Layout.ActionPillHeight))
        {
            CopyRange(low, high);
        }
    ...
    private void DrawLineCard(int index, float height)
    {
        ...
        using (Fonts.PushCaption())
        {
            ...
            var meta = line.Repeat > 1
                ? string.Concat(inspectedStamp, " · ", line.Source, " · ", Loc.T(L.Console.Repeated, line.Repeat))
        ...
        if (IconButton.Draw(FontAwesomeIcon.Times, "##asl_console_inspect_close", buttonSize, Styling.TextDim, Loc.T(L.Console.Close)))
        {
            ClearSelection();
        }

        ImGui.SetCursorScreenPos(new Vector2(closeX - buttonSize - HeaderGap * scale * 0.5f, headerMidY - buttonSize * 0.5f));
        if (IconButton.Draw(FontAwesomeIcon.Copy, "##asl_console_inspect_copy", buttonSize, Styling.AccentStarSoft, Loc.T(L.Console.CopyLine)))
        {
    ...
    private void DrawStatus()
    {
        ...
        using (Fonts.PushCaption())
        {
            ...
            var hint = Loc.T(L.Console.Shortcuts);
    ...
    {
        var scale = ImGuiHelpers.GlobalScale;
        var showing = Loc.T(L.Console.Showing, viewCount.ToString("N0", Loc.Culture), bufferedCount.ToString("N0", Loc.Culture));
        ...
        var reset = Loc.T(L.Console.ResetFilters);
        ...
        var hit = Hit.Area("##asl_console_reset", resetSize);
        var color = hit.Hovered ? Styling.AccentStarSoft : Styling.AccentStar;
