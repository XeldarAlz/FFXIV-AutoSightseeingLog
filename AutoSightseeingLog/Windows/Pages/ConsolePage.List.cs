...
internal sealed partial class ConsolePage
{
    ...
    private const string ListId = "##asl_console_list";
    private const string MenuId = "##asl_console_menu";
    ...
    private void DrawRow(ImDrawListPtr dl, int index, Vector2 rowMin, Vector2 rowSize, Columns columns, bool hovered)
    {
        ...
        if (selected)
        {
            Paint.Fill(dl, rowMin, rowMax, Styling.WithAlpha(Styling.AccentStar, hovered ? 0.22f : 0.16f), rounding);
            Paint.Fill(dl, rowMin, new Vector2(rowMin.X + SelectionBarWidth * scale, rowMax.Y), Styling.AccentStar, rounding);
        }
        else if (hovered)
        {
    ...
    private void DrawTrailing(ImDrawListPtr dl, in RunLogLine line, string? repeatLabel, Vector2 rowMax, float rowHeight, bool hovered)
    {
        ...
        if (repeatLabel is not null)
        {
            ...
            {
                Tooltip.Show(Loc.T(L.Console.Repeated, line.Repeat));
            }
        ...
        {
            Tooltip.Show(Loc.T(L.Console.HasDetails));
        }
    }
    ...
    private void DrawJumpPill(float scrollY, float visibleHeight, float innerWidth)
    {
        ...
        var label = newWhileDetached > 0
            ? string.Concat(Loc.T(L.Console.JumpLatest), " · ", Loc.Plural(L.Console.NewLines, newWhileDetached))
            : Loc.T(L.Console.JumpLatest);
        ...
        if (PillButton.Draw("##asl_console_jump", label, Styling.AccentStar, PillButton.Emphasis.Filled, FontAwesomeIcon.ArrowDown, height: Layout.ActionPillHeight))
        {
            follow = true;
            newWhileDetached = 0;
        }
    ...
    private void DrawContextMenu()
    {
        ...
        if (ImGui.MenuItem(Loc.T(L.Console.CopyLine)))
        {
            CopyRange(contextIndex, contextIndex);
        }

        var (low, high) = SelectionIndices();
        if (high > low && ImGui.MenuItem(string.Concat(Loc.T(L.Console.CopySelection), " (", Loc.Plural(L.Console.Selected, high - low + 1), ")")))
        {
            CopyRange(low, high);
        }

        if (ImGui.MenuItem(Loc.T(L.Console.CopyToEnd)))
        {
            CopyRange(contextIndex, viewCount - 1);
        }

        ImGui.Separator();
        if (ImGui.MenuItem(Loc.T(L.Console.OnlySource, line.Source), false, source is null))
        {
            source = line.Source;
            ClearSelection();
        }
    ...
    private void DrawEmpty()
    {
        ...
        if (bufferedCount > 0)
        {
            var noMatches = Loc.T(L.Console.NoMatches);
            ...
            if (PillButton.Draw("##asl_console_reset_empty", Loc.T(L.Console.ResetFilters), Styling.AccentStar, PillButton.Emphasis.Tinted, FontAwesomeIcon.Undo,
        ...
        var empty = Loc.T(L.Console.Empty);
        ...
        var footer = Loc.T(L.Console.Footer, AslConstants.LogPrefix);
