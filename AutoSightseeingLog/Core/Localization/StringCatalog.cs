...
internal sealed class StringCatalog
{
    ...
    public static StringCatalog Load(string path)
    {
        ...
        {
            Svc.Log.Error(exception, $"{AslConstants.LogPrefix} Failed to load language catalog '{path}'");
            return Empty;
        }
    ...
    public static ushort[] ScanGlyphRanges(string path)
    {
        ...
        {
            Svc.Log.Error(exception, $"{AslConstants.LogPrefix} Failed to scan glyphs from '{path}'");
            return [0];
        }
