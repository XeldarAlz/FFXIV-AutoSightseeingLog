...
internal static class LocAudit
{
    ...
    public static void Run(string directory)
    {
        ...
        for (var index = 0; index < Languages.All.Length; index++)
        {
            ...
            if (catalog.Count == 0)
            {
                Svc.Log.Warning($"{AslConstants.LogPrefix} [Loc] '{language.Code}.json' missing or empty.");
                continue;
            }
            ...
            if (missingCount == 0)
            {
                Svc.Log.Info($"{AslConstants.LogPrefix} [Loc] '{language.Code}.json' complete ({keys.Count} keys).");
                continue;
            }

            Svc.Log.Warning($"{AslConstants.LogPrefix} [Loc] '{language.Code}.json' missing {missingCount}/{keys.Count} keys: {missing}…");
        }
    }
