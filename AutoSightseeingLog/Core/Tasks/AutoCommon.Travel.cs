...
internal abstract partial class AutoCommon
{
    ...
    private async Task RecoverFromFalseStart(string scope, int falseStarts)
    {
        ...
        Svc.Chat.PrintError($"{AslConstants.LogPrefix} The character keeps moving on its own, so paths are dropped as soon as they start. Turn off the pathfinder's 'cancel path on movement input' setting, and check that no key or auto-run is held.");
    }
    ...
    // Log and approach points can sit a little inside the terrain or above it; the floor under them is what the pathfinder accepts.
